...
async def scrape_and_extract_data_v1(params: UrlAnalysisRequestParams) -> UrlAnalysisResponseModel:
    extracted_data: dict[str, str] = {key: "" for key in params.sought_data}
    links_to_visit: set[HttpUrl] = {params.url}
    visited_links = set()
    while len(links_to_visit) > 0:
    ...
    # extract data
    new_data: Dict[str, str] = await extract_information_from_url(params, []) # TODO: populate the url list
    extracted_data = update_dict_with_new_data(extracted_data, new_data)
